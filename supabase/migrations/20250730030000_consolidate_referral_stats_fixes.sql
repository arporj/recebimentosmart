-- supabase/migrations/20250730030000_consolidate_referral_stats_fixes.sql

-- Correção consolidada da função get_referral_stats
...
-- Correção consolidada da função get_full_referral_stats
...
LANGUAGE plpgsql
SECURITY DEFINER -- Adicionado SECURITY DEFINER para garantir permissões
AS $$
BEGIN
  RETURN QUERY
  WITH user_referral_code AS (
    -- Pega o código de indicação do próprio usuário da tabela profiles
  ...
  referral_counts AS (
    ...
    WHERE u.raw_user_meta_data->>'referral_code' = (SELECT referral_code FROM user_referral_code)
  ),
  referral_info AS (
  ...
  SELECT
    (SELECT referral_code FROM user_referral_code) AS referral_code,
