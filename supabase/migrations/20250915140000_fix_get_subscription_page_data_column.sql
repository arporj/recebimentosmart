-- Migration: Corrige o nome da coluna na função `get_subscription_page_data`.
--
-- Motivo:
-- A versão anterior continha um erro de digitação, referenciando `p.plan` em vez de `p.plano`,
-- o que causava um erro 400 (Bad Request) ao chamar a função.
...
BEGIN
    ...
    result := jsonb_build_object(
        ...
        'user', jsonb_build_object(
            'plan', user_plan_details.plano, -- CORRIGIDO: `plan` para `plano`
