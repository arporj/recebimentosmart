-- Migration: Reescreve a função `get_all_users_admin` para usar a tabela `subscriptions`.
--
-- Motivo:
-- A lógica de planos e status foi movida para a tabela `subscriptions`.
-- A função precisa agora buscar o plano, status e data de validade da assinatura
-- mais recente e ativa de cada usuário, em vez da tabela `profiles`.
