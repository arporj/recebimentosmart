...
BEGIN
  ...
  WITH user_referral_code AS (
    -- Pega o código de indicação do próprio usuário da tabela profiles e converte para TEXT
    SELECT p.referral_code::TEXT AS code_value
  ...
  referral_counts AS (
    ...
    SELECT
      COUNT(*) AS registered,
      COUNT(CASE WHEN EXISTS (SELECT 1 FROM public.payments WHERE payments.user_id = u.id) THEN 1 END) AS paid
    FROM auth.users u
    LEFT JOIN public.profiles p ON u.id = p.id
    WHERE u.raw_user_meta_data->>'referral_code' = (SELECT code_value FROM user_referral_code)
  ),
  referral_info AS (
  ...
  SELECT
    (SELECT code_value FROM user_referral_code) AS referral_code,
