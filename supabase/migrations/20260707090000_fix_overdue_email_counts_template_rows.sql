-- Migração: corrige falsos positivos no e-mail de "Contas em Atraso"
--
-- process_due_accounts_notifications() e process_due_accounts_notification_test()
-- usavam uma heurística frágil para excluir o template de uma recorrência (is_template=true,
-- parent_id IS NULL) da lista de lançamentos vencidos: só excluía o template se ele já tivesse
-- um filho 'paid'/'cancelled' no mesmo período. Como o padrão atual de recorrência
-- (20260701160000_refactor_recurrence_to_template_pattern.sql) já gera as ocorrências reais
-- como linhas próprias (is_template=false) no momento da criação/edição, o template nunca
-- deveria aparecer como um lançamento vencido por si só — mas enquanto a ocorrência real do
-- período ainda estiver pending (o caso comum), a heurística falhava em excluir o template,
-- fazendo ele contar como um vencido "fantasma" duplicado ao lado da ocorrência real.
--
-- Fix: substitui a heurística por um filtro direto "ft.is_template = false", que é o mesmo
-- critério já usado em todo o resto do app (executor.ts do Artie, FinancialTransactionsV2, etc.)
-- para distinguir lançamentos reais de templates de recorrência.
