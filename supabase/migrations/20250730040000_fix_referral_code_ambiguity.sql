-- supabase/migrations/20250730040000_fix_referral_code_ambiguity.sql

-- Correção da função get_full_referral_stats para resolver ambiguidade de coluna
...
BEGIN
  ...
  WITH user_referral_code AS (
    ...
    SELECT p.referral_code AS user_ref_code -- Alias para evitar ambiguidade
  ...
  SELECT
    (SELECT user_ref_code FROM user_referral_code) AS referral_code, -- Usa o alias aqui
