-- Atualiza a função seed_default_categories para incluir a categoria 'Acerto de Saldo' para novos usuários
-- Usada pelo fechamento de fatura de cartão de crédito quando o valor final difere do total calculado.
...
BEGIN
    INSERT INTO public.financial_categories (user_id, name, parent_id, icon) VALUES
        ...
        (p_user_id, 'Pet', NULL, '🐾'),
        (p_user_id, 'Acerto de Saldo', NULL, '⚖️'),
        (p_user_id, 'Outros', NULL, '📦')
    ON CONFLICT (user_id, name, parent_id) DO NOTHING;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Adiciona a categoria 'Acerto de Saldo' para todos os usuários existentes no banco de dados
INSERT INTO public.financial_categories (user_id, name, parent_id, icon)
SELECT id, 'Acerto de Saldo', NULL, '⚖️'
FROM auth.users
ON CONFLICT (user_id, name, parent_id) DO NOTHING;
