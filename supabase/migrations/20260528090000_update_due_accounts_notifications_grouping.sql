-- =============================================================================
-- Migração: Reescreve a RPC de notificação de contas pendentes para:
--   1. Agrupar lançamentos por conta financeira (excluindo lançamentos de cartão)
--   2. Mostrar transferências entre contas (saída na origem, entrada no destino)
--   3. Colorir valores (verde para entrada, vermelho para saída) sem badges
--   4. Incluir logo SVG + texto estilizado no cabeçalho
-- =============================================================================

-- 1. Atualiza a função principal de disparo de e-mails
...
BEGIN
    ...
    LOOP
        -- Busca a configuração de e-mail do plano do usuário
        ...
        LOOP
            ...
            -- 1. Lançamentos de income/expense desta conta (excluindo os de contas de cartão de crédito)
            FOR v_tx IN
            ...
            -- 2. Transferências SAINDO desta conta (account_id = conta atual)
            FOR v_tx IN
            ...
            -- 3. Transferências ENTRANDO nesta conta (destination_account_id = conta atual)
            FOR v_tx IN
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr>
                                ...
                                <th style="padding: 8px 12px; text-align: center; font-size: 11px; font-weight: bold; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 2px solid #e2e8f0;">Data</th>
                            </tr>
                        </thead>
                        <tbody>
                            ' || v_account_rows || '
                ...
                <div style="max-width: 600px; margin: 30px auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
                    ...
                    <div style="background-color: #0f172a; padding: 24px; text-align: center; border-bottom: 3px solid #14b8a6;">
                        ...
                        <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.1);">
                            <h1 style="color: #ffffff; margin: 0; font-size: 18px; font-weight: bold; letter-spacing: 0.3px;">' || CASE WHEN v_frequency = 'weekly' THEN 'Seu Resumo Semanal Financeiro' ELSE 'Alerta de Contas Hoje' END || '</h1>
                            <p style="color: #94a3b8; margin: 4px 0 0 0; font-size: 12px;">Acompanhe suas contas pendentes e mantenha seu fluxo saudável</p>
                        </div>
                    </div>
                    ...
                    <div style="padding: 24px; color: #334155; line-height: 1.6;">
                        ...
                        <p style="font-size: 13px; color: #64748b; margin-bottom: 24px;">
                            Identificamos <strong>' || v_total_tx_count || '</strong> lançamento(s) pendente(s) com vencimento ' || v_period_label || '. Veja o resumo por conta:
                        </p>

                        ' || v_account_section || '
                        
                        <div style="text-align: center; margin-top: 28px;">
                            <a href="https://recebimentosmart.com.br/dashboard" style="display: inline-block; padding: 12px 28px; background-color: #14b8a6; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 13px; box-shadow: 0 4px 6px rgba(20, 184, 166, 0.2);">Acessar Meu Painel Financeiro</a>
                        </div>
                    </div>
...
-- =============================================================================
-- 2. Nova RPC para teste: envia o e-mail de contas de HOJE para andre@andreric.com
--    com os dados de um usuário específico
-- =============================================================================
...
BEGIN
    ...
    IF v_total_tx_count = 0 THEN
        RETURN jsonb_build_object('success', true, 'message', 'Nenhum lançamento pendente para hoje.', 'count', 0);
    END IF;
    ...
    <body style="font-family: Arial, sans-serif; margin: 0; padding: 0; background-color: #f1f5f9;">
        <div style="max-width: 600px; margin: 30px auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
            ...
            <div style="background-color: #0f172a; padding: 24px; text-align: center; border-bottom: 3px solid #14b8a6;">
                ...
                <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.1);">
                    <h1 style="color: #ffffff; margin: 0; font-size: 18px; font-weight: bold; letter-spacing: 0.3px;">Alerta de Contas Hoje</h1>
                    <p style="color: #94a3b8; margin: 4px 0 0 0; font-size: 12px;">Acompanhe suas contas pendentes e mantenha seu fluxo saudável</p>
                </div>
            </div>
            ...
            <div style="padding: 24px; color: #334155; line-height: 1.6;">
                <p style="font-size: 15px; font-weight: bold; color: #1e293b; margin-top: 0;">Olá, ' || COALESCE(v_user.name, 'Usuário') || '!</p>
                <p style="font-size: 13px; color: #64748b; margin-bottom: 24px;">
                    Identificamos <strong>' || v_total_tx_count || '</strong> lançamento(s) pendente(s) com vencimento hoje. Veja o resumo por conta:
                </p>
