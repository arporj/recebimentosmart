-- Migração: corrige falso positivo de "Contas em Atraso" no e-mail diário/semanal
--
-- process_due_accounts_notifications() e process_due_accounts_notification_test() decidem se o
-- e-mail é "Contas em Atraso" ou "Contas a Vencer Hoje" checando se existe QUALQUER lançamento
-- pendente com data < hoje (v_has_overdue). Essa checagem não excluía lançamentos de despesa de
-- cartão de crédito (ft.account_id apontando pra uma conta type='credit_card'): a data desses
-- lançamentos é o dia da COMPRA, que é sempre no passado em relação ao vencimento real da fatura
-- (calculado por invoice_month/due_day). Qualquer usuário com uma compra de cartão pendente já
-- disparava v_has_overdue=true e o e-mail virava "Contas em Atraso" incorretamente, mesmo quando
-- a única conta realmente vencendo era de hoje.
--
-- Fix: exclui lançamentos cuja conta é cartão de crédito da checagem de existência de atraso
-- (o controle de fatura de cartão já é feito à parte, por invoice_month/due_day).
--
-- Também troca `(CURRENT_DATE AT TIME ZONE 'America/Sao_Paulo')::date` por
-- `(now() AT TIME ZONE 'America/Sao_Paulo')::date` para calcular "hoje": CURRENT_DATE já é uma
-- data sem fuso horário, então aplicar AT TIME ZONE nela não converte corretamente o horário atual
-- para o fuso de Brasília — o resultado correto depende do fuso da sessão do banco coincidir com
-- o esperado. Partir de now() (um instante com fuso definido) é a forma correta e inequívoca de
-- obter a data corrente em America/Sao_Paulo.
...
BEGIN
    ...
    LOOP
        ...
        IF v_frequency = 'weekly' THEN
            ...
            -- e excluindo despesas de cartão de crédito, cuja data é a da compra e não a do vencimento da fatura)
            SELECT EXISTS (
                SELECT 1
                FROM public.financial_transactions ft
                LEFT JOIN public.financial_accounts fa ON fa.id = ft.account_id
                WHERE ft.user_id = v_user.id
                  ...
                  AND COALESCE(fa.type, '') <> 'credit_card'
            ) INTO v_has_overdue;
