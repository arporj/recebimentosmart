...
-- Agora, a função consulta diretamente a tabela net._http_response para obter o status e o corpo da resposta.
...
BEGIN
    ...
    -- 2. Aguarda um curto período para a requisição ser processada e o resultado estar disponível na tabela de respostas.
    ...
    IF response_status_code IS NOT NULL THEN
        ...
        END IF;
    ELSE
        RAISE WARNING 'Resposta da Edge Function não disponível em net._http_response após 5 segundos. Request ID: %', request_id;
    END IF;
