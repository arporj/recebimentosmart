...
### 30. 💳 Total Previsto no Agrupamento de Contas (Tela de Lançamentos — Backlog 1.7)
* **Status:** Concluído em Jul/2026.
* **Descrição:** No card "Contas" da barra lateral de resumo do `FinancialTransactionsV2.tsx`, o cabeçalho de cada grupo (Conta Corrente/Poupança/Investimento) mostrava apenas `group.confirmed` — o saldo somando só lançamentos já confirmados até o fim do mês. O cálculo do saldo previsto por grupo (`group.projected`, agregando `computeAccountBalanceAsOf` com `onlyConfirmed: false` até `endOfMonth(currentMonth)`) já existia e era usado nas linhas individuais de cada conta — só faltava exibi-lo no cabeçalho do grupo. Trocado o valor principal do cabeçalho para `group.projected` (verde/vermelho conforme sinal), mantendo `group.confirmed` como texto secundário ("Conf: X") logo abaixo, para não perder a informação anterior. Mudança puramente visual, sem nova query nem alteração em `computeAccountBalanceAsOf`.

### 31. 📋 Clonar Lançamento Abre a Tela de Criação em vez de Salvar Direto (Backlog 1.7.3)
* **Status:** Concluído em Jul/2026.
* **Descrição:** O botão "Clonar" em `FinancialTransactionsV2.tsx` (compartilhado entre a lista mobile e a tabela desktop, ambos chamando o mesmo `handleClone`) inseria um novo lançamento pendente direto no banco, copiando apenas alguns campos e ignorando tags, cartão, parcelamento/recorrência. Agora `handleClone` abre `FinancialTransactionModalV2` pré-preenchido com todos os dados do lançamento original (via o mesmo `transaction` prop usado por `handleEdit`), permitindo revisar/ajustar antes de confirmar a criação. Para isso, o modal ganhou um prop `isCloning` que desacopla `isEditing` de `!!transaction` (`isEditing = !!transaction && !isCloning`), fazendo o formulário popular todos os campos (inclusive tags) mas submeter como criação nova (`criarTransacaoFinanceira`), sem exibir o botão "Excluir" nem travar modalidade/parcelas. O clone é pré-preenchido com a descrição idêntica ao original (sem sufixo) e status forçado para pendente, mesmo que o original esteja pago. Validado em navegador (Playwright) nos dois layouts responsivos (desktop ≥1280px e mobile <768px): abertura do modal pré-preenchido, ausência do botão Excluir, criação efetiva de um novo lançamento independente sem alterar o original, e cancelamento sem gravar nada.
