...
const PriceManagement = () => {
  ...
  const handleUpdatePrice = async () => {
    ...
    const isConfirmed = window.confirm(
      `Você tem certeza que deseja alterar o preço para R$ ${newPrice}? Todos os usuários ativos serão notificados por e-mail.`
    );
    ...
    setUpdating(true);
    try {
      const { data, error } = await supabase.rpc('update_price_and_notify', { new_price: newPrice });
  ...
  return (
    <div className="space-y-6">
      ...
      <button
        ...
        disabled={loading || updating || newPrice === price}
